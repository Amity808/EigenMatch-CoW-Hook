...
import {IEigenMatchHook} from "@project/interfaces/IEigenMatchHook.sol";
...
contract DeployMyL2Contracts is Script {
    ...
    function run(string memory environment, string memory _context) public {
        ...
        address hookAddress = vm.envAddress("EIGENMATCH_HOOK_ADDRESS");
        EigenMatchDigestRegistry registry = EigenMatchDigestRegistry(_readCustomL1Address(environment, "EigenMatchDigestRegistry"));

        address[] memory watchers = new address[](1);
        watchers[0] = context.avs;

        EigenMatchSettlementExecutor executor =
            new EigenMatchSettlementExecutor(IEigenMatchHook(hookAddress), registry, context.avs, watchers, 1);
        console.log("EigenMatchSettlementExecutor deployed to:", address(executor));
        ...
        outputs[0] = Output({name: "EigenMatchSettlementExecutor", contractAddress: address(executor)});
