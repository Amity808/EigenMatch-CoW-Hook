...
The template ships with Hourglass sample contracts (`TaskAVSRegistrar`, `AVSTaskHook`, etc.) so DevKit can compile/deploy during devnet boot. We layer EigenMatch-specific glue on top:

- `l1-contracts/EigenMatchDigestRegistry.sol` – publishes the EigenCompute docker digest + TEE measurement allowlist that operators/watchers reference before accepting a bundle.
- `l2-contracts/EigenMatchSettlementExecutor.sol` – verifies watcher signatures and registry state before calling `EigenMatchHook.processSettlementBundle`.

These sit alongside the Hourglass infra (registrar, task hook) so devnet spins up end-to-end while still reflecting the intent-matching requirements in `README.md`.

## What’s left to align with the root README?

- Port the real EigenMatch settlement contracts into `contracts/src/**`.
- Update `.devkit/scripts/deploy*.s.sol` to emit JSON artifacts consumed by the performer (hook addresses, fee configs, etc.).
- Wire the watcher/executor bridge so validated bundles automatically hit the Uniswap hook rather than just being logged.
- Expand Go/Forge tests (`cmd/main_test.go`, `contracts/test/*.t.sol`) to cover the EigenMatch-specific invariants listed in `docs/prd.md`.
